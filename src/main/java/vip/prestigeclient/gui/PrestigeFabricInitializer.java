...
public class PrestigeFabricInitializer implements ClientModInitializer {
    ...
    public void onInitializeClient() {
        openGuiBind = KeyBindingHelper.registerKeyBinding(new KeyBinding(
            "key.prestigeclient.opengui",
            InputUtil.Type.KEYSYM,
            GLFW.GLFW_KEY_RIGHT_SHIFT,
            "category.prestigeclient.binds"
        ));
        ...
        ClientTickEvents.END_CLIENT_TICK.register(client -> {
            while (openGuiBind.wasPressed()) {
                System.out.println("Prestige ClickGUI Toggle Action Triggered!");
            }
        });
    }
}
